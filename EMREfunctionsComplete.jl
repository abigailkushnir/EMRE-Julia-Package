...
function adjustParams(rn::ReactionSystem, params, volume)
    ...
    for reaction in systemReactions
        ...
        if length(reaction.substrates) == 2 ## check if it's a bimolecular reaction
            paramIndex = findfirst(x->isequal(x,reaction.rate), systemParams) ## if it is, find what number parameter it is
            if !adjustedTF[paramIndex] ## if it hasn't already been adjusted
        ...
        if (length(reaction.substrates) == 1) && (reaction.substoich[1] == 2) ## check if it's a bimolecular reaction with same-species
            paramIndex = findfirst(x->isequal(x,reaction.rate), systemParams) ## if it is, find what number parameter it is
            if !adjustedTF[paramIndex] ## if it hasn't already been adjusted
...
function runLNA(rn::ReactionSystem, ics, params, volume, tEnd, numSteps)
    ...
    lna = LNASystem(rn);
    prob = ODEProblem(lna, ics/volume, tSpan, params);
...
function buildDeltaVect(rn::ReactionSystem, params, meanIdxs, varIdxs, LNAsol)
    ...
    for reaction in rn.rxs
        ...
        if length(reaction.substrates) == 2 ## if a bimolecular reaction of different species
        ...
        if length(reaction.substrates) == 1 && (reaction.substoich[1] == 2) ## if it's a bimolecular reaction with the same species
...
function buildNewDESystem(rn::ReactionSystem, conservationLaws)
    ...
    @named newSystem = ODESystem([substitute(eq, subDict) for eq in reducedSystem]) ## convert the remaining DEs to an ODESystema
